...
The fact that Java has a HashMap
class means
that no
Java programmer
has to
...
not an
exercise in
generic programming;do not try
to write
a generic class.)
Write an
implementation of
...
The hash code is used in all the methods that deal with keys, to decide which linked list to look at. Once a list has been selected, the operations on the list are pretty

    straightforward (given all the information and code in section 9.2.2 of the textbook), so they will not be discussed further here. You can look at the solution, below.

Although it is not required by the exercise,

    a resize() method was defined that is used to increase the size of the table when the table becomes too full. This method is called in the put() method when the table becomes more than 3/4 full.

A major part of developing a class for general use is testing. It's important to design a testing procedure that will test all aspects of the class. For this problem, a testing program was written that would allow you to test each of the methods in the class. Also added is a
