...
// * (to its right). The pointers will allow the machine to advance from one
// cell
...
// /**
// * Moves the current cell one position to the left along the tape. Note that
// if
...
// * tape is conceptually infinite, so your linked list must be prepared to
// expand
...
// /**
// * Moves the current cell one position to the right along the tape. Note that
// if
// * the current cell is the rightmost cell that exists, then a new cell must be
// * created and added to the tape at the right of the current cell, and then
// the
// * current cell pointer can be moved to point to the new cell. The content of
...
// current = current.next;
// }
// // If the string has a character at least and the last character is blank
// space,
