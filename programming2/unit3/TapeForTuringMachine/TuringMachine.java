...
public class TuringMachine {
	 ...
	 *             value will not cause an immediate error, but will produce
	 *             NullPointExceptions
	 ...
	 *             the current cell on the tape and the content of all the cells on
	 *             the
	 ...
	 *                               no applicable rule can be found. This is taken
	 *                               to indicate a bug in the
	 *                               machine's program.
	 */
	public String run(Tape tape) throws IllegalStateException {
		...
		while (currentState >= 0) {
			...
			if (applicableRule.moveLeft)
				...
				tape.moveRight();
			// System.out.println(applicableRule.currentState + " "+
			// applicableRule.currentContent
